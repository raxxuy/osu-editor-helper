...
import "./App.css";
import { BeatmapEditor } from "./components/beatmap";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const waitForChange = async () => {
      if (file) {
        await watchImmediate(file.fullPath, async (_event) => {
          const updatedContent = await readTextFile(file.fullPath);
  ...
  return (
    <div className="flex h-screen w-full items-center justify-center bg-[#1c1719] text-white">
      {beatmapData ? (
        <BeatmapEditor beatmapData={beatmapData} />
      ) : (
